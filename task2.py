...
#    print(x)
    df = prepareDF(x[0]).withColumn('category1',lit(x[1])).withColumn('category2',lit(x[2])).withColumn('column_id',lit( column_id ))
...
def get_train_set(trial_num ):
    ...
    while len(label_set) < len(categories) - 1 :   # make sure each category appears at least once
        train_set.clear()
...
count_vectors = CountVectorizer(inputCol='words', outputCol='features', vocabSize=1000000, minDF=1)
...
tmp_row = pred.select('label','category1').distinct().collect()
for x in tmp_row:
    mapping[x[0]] = x[1]
...
for x in data[0:trial_num]: ##########################################################   testtest 
...
######################
    double_label = False      ##  whether two labels
#############################
...
#    result_df2.show()
    ...
    print('column '+ data.index(x) + ' complete')
